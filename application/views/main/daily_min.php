...
<body>
    ...
    <script>
	$.getJSON("<?=$this->config->item('base_api')?>api2/dustboy/stations", function(db){if (db){<?php  if($_pmType=="thAQI"){?> $.each(db, function(index, value){var marker=L.marker([value.dustboy_lat, value.dustboy_lon],{icon: L.divIcon({className: "my-custom-pin", iconSize: [35, 35], iconAnchor: [0, 0], labelAnchor: [-6, 0], popupAnchor: [17, 0], html: '<div class="signoutz-marker"style="background-color:rgba(' + value.daily_th_color + ')">' + value.daily_pm25_th_aqi + '</div>'})}).addTo(map); marker.on('click', function(e){<?php  if($_lang=="thailand"){?> $('#popupDetail p').html(value.dustboy_name); <?php }else{?> $('#popupDetail p').html(value.dustboy_name_en); <?php }?> $('#popupDetail .card-header').css("background-color", "rgba(" + value.daily_th_color + ", 0.74)"); $('#popupDetail .card-body').css("background-color", "rgba(" + value.daily_th_color + ", 0.74)"); $('#popupDetail .card-footer').css("background-color", "rgba(" + value.daily_th_color + ", 0.74)"); $('#popupDetail .number_title').html(value.daily_pm25_th_aqi); $('#popupDetail .number_footer').html('PM<sub>2.5</sub><?=$_pmType=="thAQI"?" TH AQI":" US AQI"?>'); <?php  if($_lang=="thailand"){?> $('#popupDetail .detail_title').html(value.daily_th_title); <?php }else{?> $('#popupDetail .detail_title').html(value.daily_th_title_en); <?php }?> $('#popupDetail .card-body .anime img').attr("src", '/template/image/' + value.daily_th_dustboy_icon + '.svg'); $('#popupDetail .card-footer span.temp').html(value.daily_temp + ' &#8451'); $('#popupDetail .card-footer span.humid').html(value.daily_humid + ' %'); $('#popupDetail .card-footer span.pm25').html(value.daily_pm25 + ' μg/m<sup>3</sup>'); $('#popupDetail .card-footer span.pm10').html(value.daily_pm10 + ' μg/m<sup>3</sup>'); $('#popupDetail').show();});}); <?php }else{?> $.each(db, function(index, value){var marker=L.marker([value.dustboy_lat, value.dustboy_lon],{icon: L.divIcon({className: "my-custom-pin", iconSize: [35, 35], iconAnchor: [0, 0], labelAnchor: [-6, 0], popupAnchor: [17, 0], html: '<div class="signoutz-marker"style="background-color:rgba(' + value.daily_us_color + ')">' + value.daily_pm25_us_aqi + '</div>'})}).addTo(map); marker.on('click', function(e){<?php  if($_lang=="thailand"){?> $('#popupDetail p').html(value.dustboy_name); <?php }else{?> $('#popupDetail p').html(value.dustboy_name_en); <?php }?> $('#popupDetail .card-header').css("background-color", "rgba(" + value.daily_us_color + ", 0.74)"); $('#popupDetail .card-body').css("background-color", "rgba(" + value.daily_us_color + ", 0.74)"); $('#popupDetail .card-footer').css("background-color", "rgba(" + value.daily_us_color + ", 0.74)"); $('#popupDetail .number_title').html(value.daily_pm25_us_aqi); $('#popupDetail .number_footer').html('PM<sub>2.5</sub><?=$_pmType=="thAQI"?" TH AQI":" US AQI"?>'); <?php  if($_lang=="thailand"){?> $('#popupDetail .detail_title').html(value.daily_us_title); <?php }else{?> $('#popupDetail .detail_title').html(value.daily_us_title_en); <?php }?> $('#popupDetail .card-body .anime img').attr("src", '/template/image/' + value.daily_us_dustboy_icon + '.svg'); $('#popupDetail .card-footer span.temp').html(value.daily_temp + ' &#8451'); $('#popupDetail .card-footer span.humid').html(value.daily_humid + ' %'); $('#popupDetail .card-footer span.pm25').html(value.daily_pm25 + ' μg/m<sup>3</sup>'); $('#popupDetail .card-footer span.pm10').html(value.daily_pm10 + ' μg/m<sup>3</sup>'); $('#popupDetail').show();});}); <?php }?>}});
	</script>
</body>
