...
layerControl.addOverlay(actualLayerGroup, actualLayer), initializeLayer(actualLayerArray[actualTimeIndex]), window.setInterval(function() {
	...
	$.getJSON("https://www.cmuccdc.org/assets/api/standard_aqi.json", function(aqi) {
		var us = aqi[0].us_aqi;
		var th = aqi[0].th_aqi;
		...
		for (var i = 0; i < us['pm25'].length; ++i) {
			...
			us_aqi +='<div class="col col-md-1 detail_pm_aqi" style="background-color: rgb('+us['aqi'][i].color+');"><img width="30" src="/template/img/icons/'+us['aqi'][i].dustboy_icon+'"></div>'
		...
		for (var i = 0; i < th['pm25'].length; ++i) {
			...
			th_aqi +='<div class="col col-md-1 detail_pm_aqi" style="background-color: rgb('+th['aqi'][i].color+');"><img width="30" src="/template/img/icons/'+th['aqi'][i].dustboy_icon+'"></div>'
	...
	$.getJSON("https://www.cmuccdc.org/assets/api/genDustboyGeo.php?token="+token, function(db) {
		if (db) {
			var i=0;
			geojsonOpts = {
				pointToLayer: function(feature, latlng) {
					...
					i++;
					if(data_type=="th-hr"){
						var LeafIcon = L.Icon.extend({options: {iconSize:[34, 34],}});
						var greenIcon = new LeafIcon({iconUrl: 'https://www.cmuccdc.org/template/img/icon/'+feature.properties.th_dustboy_icon});
						...
						var LeafIcon = L.Icon.extend({options: {iconSize:[34, 34],}});
						var greenIcon = new LeafIcon({iconUrl: 'https://www.cmuccdc.org/template/img/icon/'+feature.properties.us_dustboy_icon});
						return marker = L.marker(latlng, {
							icon: greenIcon
						}).on('click', function(e) {
							if(data_lang=="thailand"){
						...
						var LeafIcon = L.Icon.extend({options: {iconSize:[34, 34],}});
						var greenIcon = new LeafIcon({iconUrl: 'https://www.cmuccdc.org/template/img/icon/'+feature.properties.daily_th_dustboy_icon});
						return marker = L.marker(latlng, {
							icon: greenIcon
						}).on('click', function(e) {
							if(data_lang=="thailand"){
						...
						var LeafIcon = L.Icon.extend({options: {iconSize:[34, 34],}});
						var greenIcon = new LeafIcon({iconUrl: 'https://www.cmuccdc.org/template/img/icon/'+feature.properties.daily_us_dustboy_icon});
						return marker = L.marker(latlng, {
							icon: greenIcon
						}).on('click', function(e) {
							if(data_lang=="thailand"){
