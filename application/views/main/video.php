		...
		<div class="container">
			...
			<div class="row" id="news-page-news-feed">
				<?php foreach($rsVideo as $item){?>
				<div class="col-lg-4 col-md-6 news-slice">
                    <div class="single-news-content">
                        <a href="<?=base_url('videodetail/'.$item->id)?>" class="news-thum news-thumbg-1" style="background-image: url(<?=base_url()?>uploads/timthumb.php?src=<?=$item->yt_thumbnail?>&w=740&h=530);"></a>
                        <div class="news-contant">
							<div class="new-title">
								<h4><a href="<?=base_url('videodetail/'.$item->id)?>"><?=$item->yt_name?></a></h4>
							</div>
                            <p class="news-meta">
                                <a href="<?=base_url('videodetail/'.$item->id)?>"><i class="fa fa-calendar"></i> <?=ConvertToThaiDate($item->yt_datetime,1)?></a>
                                <a href="<?=base_url('videodetail/'.$item->id)?>" style="float: right;" class="alignright rd-btn">อ่านเพิ่ม <i class="fas fa-arrow-right"></i></a>
                            </p>
                        </div>
                    </div>
                </div>
