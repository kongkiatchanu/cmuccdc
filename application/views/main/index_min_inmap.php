...
#floating-panel .table{color:#fff;}
#floating-panel .table td{padding: 3px;}
	</style>
    <div id="btn_contact">
        <span></span>
        <span></span>
							...
							<ul>
								<li>
									...
									<table class="table table-bordered">
										...
										<tr>
											<td class="text-left">PM<sub>2.5</sub> (μg/m<sup>3</sup>)</td>
											<td style="background-color: rgb(0, 191, 243);">0-25</td>
											<td style="background-color: rgb(0, 166, 81);">26-37</td>
											<td style="background-color: rgb(255, 242, 0);color:#333">38-50</td>
											<td style="background-color: rgb(242, 101, 34);">51-180</td>
											<td style="background-color: rgb(255, 0, 0);">>180</td>
										</tr>
										<tr>
											<td class="text-left">TH AQI</td>
											<td style="background-color: rgb(0, 191, 243);">0-25</td>
											<td style="background-color: rgb(0, 166, 81);">26-50</td>
											<td style="background-color: rgb(255, 242, 0);color:#333">51-100</td>
											<td style="background-color: rgb(242, 101, 34);">101-200</td>
											<td style="background-color: rgb(255, 0, 0);">>200</td>
										</tr>
										</tbody>
									</table>
									<?php }else{?>
									<table class="table table-bordered">
										...
										<tr>
											<td class="text-left">PM<sub>2.5</sub> (μg/m<sup>3</sup>)</td>
											<td style="background-color: rgb(0, 153, 107);">0-11.9</td>
											<td style="background-color: rgb(255, 220, 90);color:#333">12-35.4</td>
											<td style="background-color: rgb(235, 132, 63);">35.5-55.4</td>
											<td style="background-color: rgb(209, 1, 53);">55.5-150.4</td>
											<td style="background-color: rgb(129, 21, 185);">150.5-250.4</td>
											<td style="background-color: rgb(160, 7, 54);">>250.4</td>
										</tr>
										<tr>
											<td class="text-left">US AQI</td>
											<td style="background-color: rgb(0, 153, 107);">0-50</td>
											<td style="background-color: rgb(255, 220, 90);color:#333">51-100</td>
											<td style="background-color: rgb(235, 132, 63);">101-150</td>
											<td style="background-color: rgb(209, 1, 53);">151-200</td>
											<td style="background-color: rgb(129, 21, 185);">201-300</td>
											<td style="background-color: rgb(160, 7, 54);">>300</td>
										</tr>
										</tbody>
									</table>
									<?php }?>
    ...
    <script>$.getJSON("<?=$this->config->item('base_api')?>api2/dustboy/stations", function(db){if (db){<?php  if($_pmType=="thAQI"){?> $.each(db, function(index, value){var marker=L.marker([value.dustboy_lat, value.dustboy_lon],{icon: L.divIcon({className: "my-custom-pin", iconSize: [35, 35], iconAnchor: [0, 0], labelAnchor: [-6, 0], popupAnchor: [17, 0], html: '<div class="signoutz-marker"style="background-color:rgba(' + value.th_color + ')">' + value.th_aqi + '</div>'})}).addTo(map); marker.on('click', function(e){<?php  if($_lang=="thailand"){?> $('#popupDetail p').html(value.dustboy_name); <?php }else{?> $('#popupDetail p').html(value.dustboy_name_en); <?php }?> $('#popupDetail .card-header').css("background-color", "rgba(" + value.th_color + ", 0.74)"); $('#popupDetail .card-body').css("background-color", "rgba(" + value.th_color + ", 0.74)"); $('#popupDetail .card-footer').css("background-color", "rgba(" + value.th_color + ", 0.74)"); $('#popupDetail .number_title').html(value.th_aqi); $('#popupDetail .number_footer').html('PM<sub>2.5</sub><?=$_pmType=="thAQI"?" TH AQI":" US AQI"?>'); <?php  if($_lang=="thailand"){?> $('#popupDetail .detail_title').html(value.th_title); <?php }else{?> $('#popupDetail .detail_title').html(value.th_title_en); <?php }?> $('#popupDetail .card-body .anime img').attr("src", '/template/image/' + value.th_dustboy_icon + '.svg'); $('#popupDetail .card-footer span.temp').html(value.temp + ' &#8451'); $('#popupDetail .card-footer span.humid').html(value.humid + ' %'); $('#popupDetail .card-footer span.pm25').html(value.pm25 + ' μg/m<sup>3</sup>'); $('#popupDetail .card-footer span.pm10').html(value.pm10 + ' μg/m<sup>3</sup>'); $('#popupDetail').show();});}); <?php }else{?> $.each(db, function(index, value){var marker=L.marker([value.dustboy_lat, value.dustboy_lon],{icon: L.divIcon({className: "my-custom-pin", iconSize: [35, 35], iconAnchor: [0, 0], labelAnchor: [-6, 0], popupAnchor: [17, 0], html: '<div class="signoutz-marker"style="background-color:rgba(' + value.us_color + ')">' + value.us_aqi + '</div>'})}).addTo(map); marker.on('click', function(e){<?php  if($_lang=="thailand"){?> $('#popupDetail p').html(value.dustboy_name); <?php }else{?> $('#popupDetail p').html(value.dustboy_name_en); <?php }?> $('#popupDetail .card-header').css("background-color", "rgba(" + value.us_color + ", 0.74)"); $('#popupDetail .card-body').css("background-color", "rgba(" + value.us_color + ", 0.74)"); $('#popupDetail .card-footer').css("background-color", "rgba(" + value.us_color + ", 0.74)"); $('#popupDetail .number_title').html(value.us_aqi); $('#popupDetail .number_footer').html('PM<sub>2.5</sub><?=$_pmType=="thAQI"?" TH AQI":" US AQI"?>'); <?php  if($_lang=="thailand"){?> $('#popupDetail .detail_title').html(value.us_title); <?php }else{?> $('#popupDetail .detail_title').html(value.us_title_en); <?php }?> $('#popupDetail .card-body .anime img').attr("src", '/template/image/' + value.us_dustboy_icon + '.svg'); $('#popupDetail .card-footer span.temp').html(value.temp + ' &#8451'); $('#popupDetail .card-footer span.humid').html(value.humid + ' %'); $('#popupDetail .card-footer span.pm25').html(value.pm25 + ' μg/m<sup>3</sup>'); $('#popupDetail .card-footer span.pm10').html(value.pm10 + ' μg/m<sup>3</sup>'); $('#popupDetail').show();});}); <?php }?>}});
	</script>
</body>
