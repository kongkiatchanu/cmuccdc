...
function getTblResult(t, e, a) {
    ...
    $.getJSON(l, function(t) {
        if (t) {
            ...
            var e = "";
            $.each(t, function(t, a) {
                e += "<tr>", e += '<td><div class="db_name"><a href="<?=base_url()?>' + a.dustboy_uri + '" class="dustboy_link">' + a.dustboy_name + "</a></div></td>", e += '<td class="text-center">' + parseFloat(a.distance).toFixed(1) + "</td>", e += '<td class="text-center">' + a.pm25 + "</td>", e += '<td class="text-center"><div class="c_value" style="background-color: rgb(' + a.th_color + ')">' + a.pm25_th_aqi + "</div></td>", e += '<td class="text-center"><div class="c_value" style="background-color: rgb(' + a.us_color + ')">' + a.pm25_us_aqi + "</div></td>", e += "</tr>"
				
			const marker = L.marker([a.dustboy_lat, a.dustboy_lon], {
					icon: L.divIcon({
				...
				marker.on('click', function(e) {
					marker.bindTooltip(a.dustboy_name+ '<br/>PM<sub>2.5</sub> = '+a.pm25 + ' μg/m<sup>3</sup>').openTooltip();
				});
				markers.addLayer(marker);	
				markers.addTo(map);
            }), $("#tblResult").html('<div class="table-responsive"><table class="table table-hover"><thead><tr class="text-center"><th>สถานีเครื่องวัด</th><th width="150">ระยะห่าง(km)</th><th width="100">PM<sub>2.5</sub>(&#x03BC;g/m<sup>3</sup>)</th><th width="100">TH AQI</th><th width="100">US AQI</th></tr></thead><tbody>' + e + "</tbody></table></div>"),$("#tblResult").show()
        }
    })
}
...
function getStation(t, e, a) {
	...
	marker.on("dragend", function(l) {
		...
		circle = new L.circle(l.target.getLatLng(), {
            radius: 1e3 * a
        }); 
		
		all = L.layerGroup([marker, circle]);
		map.addLayer(all);
		t = l.target.getLatLng().lat;
		e = l.target.getLatLng().lng;
		getTblResult(l.target.getLatLng().lat, l.target.getLatLng().lng, a);
		/*
		var l = "<?=$this->config->item('base_api')?>api2/dustboy/near/" + l.target.getLatLng().lat + "/" + l.target.getLatLng().lng + "/" + a;
		$.getJSON(l, function(t) {
			if (t) {
				t.length>0? $('#update-time').html(t[0]['log_datetime']):$('#update-time').html('-');
				var e = "";
