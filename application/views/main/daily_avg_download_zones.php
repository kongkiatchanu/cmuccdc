...
<body>
		...
		<?php foreach($rsRegion as $k=>$item){?>
			<?php if($item->zone_id==$zones_id){?>
				<?php foreach($item->provinces as $provinces){?>
					<?php if($provinces->stations!=null){?>
						<h4>รายงานค่าฝุ่น PM2.5 เฉลี่ยรายวัน (ไมโครกรัมต่อลูกบาศก์เมตร) จังหวัด<?=$provinces->province_name_th?></h4>
						<table class="table table-bordered db_table">
							...
							<?php foreach($item->provinces as $province){?>
								...
								<?php if($province->province_id==$provinces->province_id){?>
