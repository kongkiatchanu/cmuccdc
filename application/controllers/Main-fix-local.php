...
class Main extends CI_Controller {
	...
	public $rsDaily;
	public $rsStat;
	private $API_URI = 'http://www-old.cmuccdc.org/api2/ccdc/';
...
//	private $ga_token = 'ya29.A0ARrdaM9pZrFSshqu_qM-4_OJ4mJvb1DPeyxmevvgHpC9_5iAkvVgoqlQSo_ML6sgFHIkZ6P_Lu2RncSdI6M8bJYfstpMWc9KgTqNQSfs_cuP75MGwCF4AofHtflOEP60KLYEP-rZtirb55EGfG-PmvzRK24CnhA';
	...
	public function hourly(){
		...
		if ( ! $this->mapList = $this->cache->get('region')){
			$this->cache->save('region',json_decode(file_get_contents('http://www-old.cmuccdc.org/assets/api/haze/pwa/json/region.json')), $time_limits);
		}
	...
	public function dustboy_profile(){
		if($this->uri->segment(1)!=null){
			...
			if($rsProfile->dustboy_id!=null){
				...
				$rsForcast = json_decode(file_get_contents('http://www-old.cmuccdc.org/api2/dustboy/forecast/'.$rsProfile->dustboy_id));
	...
	public function station(){
		if($this->uri->segment(2)!=null){
			...
			$url = 'http://www-old.cmuccdc.org/assets/api/haze/pwa/genStations_admin.php?url='.$url;
	...
	public function dailyavg(){
		...
		if ( ! $this->mapList = $this->cache->get('region_report')){
			$this->cache->save('region_report',json_decode(file_get_contents('http://www-old.cmuccdc.org/assets/api/haze/pwa/json/region_report.json')), $time_limits);
		}	
		//if ( ! $this->mapList = $this->cache->get('all')){
		//	$this->cache->save('all',json_decode(file_get_contents('https://www-old.cmuccdc.org/assets/api/haze/pwa/json/allstations.json')), $time_limits);
		//}
	...
	public function summaryreport(){
		...
		$url ='http://www-old.cmuccdc.org/assets/api/haze/pwa/getSummary.php?id='.$id;
