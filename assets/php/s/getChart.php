...
$(function () {
    ...
    $.each(names, function (i, name) {
		var url = 'json_data2.php?filename=' + name.toLowerCase() + '&local=<?=$_GET["local"]?>&key=<?=$_GET['key']?>&callback=?';
	...
	function createChart() {
        ...
        $('#chart_container').highcharts('StockChart', {
			...
			title : {text : '<?=$rs["location_id"]?> - <?=$rs["location_name"]?>'},
			yAxis: [{
				title: {
                    text: 'ug/m3'
                },
            ...
            tooltip: {
                ...
                formatter: function() {
					...
					txtshow = '<div class="tooltop '+type+'">'+  
								...
								'PM2.5 : <b>' + Highcharts.numberFormat(p2, 2) +'</b> ug/m3<br />' +
							'</div>';
