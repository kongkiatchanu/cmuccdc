<?php
	...
	if($reporttype=="h"){
		$dd = strtotime(date('Y-m-d'))-(60*60*24*1);
		$date_search =  date('Y-m-d', $dd);
	}
	$dt = $date_search;
	$de = date('Y-m-d');
...
<body>
	<div id="chart_container" height="250"><img src="/template/image/loader.gif"></div>
...
$(function () {
	...
	var chart_type = ['column', 'spline'];
    var seriesOptions = [],
        seriesCounter = 0,
        names = ["PM2.5",'AVG'];

    $.each(names, function (i, name) {
		var url = 'jsonDatabaseColumn2.php<?=$param?>&name='+name+'&key=<?=$_GET['key']?>&callback=?';
						...
						<?php if($reporttype=="h"){?>		
							Highcharts.dateFormat('%e %b %Y %H:%M:%S', new Date(this.x)) +'<br />' +
								<?php if($datatype=="pm25"){?>
									...
									'Moving AVG 24 Hour: <b>' + p2 +'</b> μg/m<sup>3</sup><br />' +
